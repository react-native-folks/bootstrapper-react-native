...
const fastlaneMobileHTTPS = 'https://mobile-credentials-repo-url-if-apply.git';
...
function configureIosFastlane() {
  return runCommand({
    ...
    .then(() => {
      ...
      const fastlaneEnv = this.fs.read(
        `${this.projectName}/ios/fastlane/config/.env`
      );
      const updateFastlaneEnv = fastlaneEnv.concat(
        `PROJECT_FILE_NAME="${
          this.projectName
        }"\nXCODE_PROJECT_PATH="${process.cwd()}/${this.projectName}/ios/${
          this.projectName
        }.xcodeproj"`
      );
      this.fs.write(
      ...
      const fastlaneAction = this.fs.read(
        `${this.projectName}/ios/fastlane/actions/project_name.rb`
      );
  ...
  return runCommand({
    ...
    .then(() => {
      const fastfile = this.fs.read(
        `${this.projectName}/android/fastlane/Fastfile.private`
      );
    ...
    .then(() => {
      const buildGradle = this.fs.read(
      ...
      updateBuildGradle = updateBuildGradle.replace(
        'versionCode 1',
      ...
      const gradleProps = this.fs.read(
